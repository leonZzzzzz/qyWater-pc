...
vjs.trigger = function(elem, event) {
  ...
  // Unscss explicitly stopped or the event does not bubble (e.g. media events)
    // recursively calls this function to bubble the event up the DOM.
    if (parent && !event.isPropagationStopped() && event.bubbles !== false) {
    vjs.trigger(parent, event);

  // If at the top of the DOM, triggers the default action unscss disabled.
 ...
 * @suppress {accessControls|checkRegExp|checkTypes|checkVars|const|constantProperty|deprecated|duplicate|es5Strict|fileoverviewTags|globalThis|invalidCasts|missingProperties|nonStandardJsDocs|strictModuleDepCheck|undefinedNames|undefinedVars|unknownDefines|usescssCode|visibility}
 */
...
vjs.Player.prototype.trackProgress = function(){
  ...
  this.progressInterval = setInterval(vjs.bind(this, function(){
    // Don't trigger unscss buffered amount is greater than last time
...
vjs.Player.prototype.controls = function(bool){
  if (bool !== undefined) {
    ...
    // Don't trigger a change event unscss it actually changed
...
// HTML5 Feature detection and Device Fixes --------------------------------- //
  ...
  // Override Android 2.2 and scss canPlayType method which is broken
...
vjs.Flash = vjs.MediaTechController.extend({
  ...
  init: function(player, options, ready){
    ...
    // Firefox 3.6 keeps calling the iframe onload function anytime I write to it, causing an endscss loop.
...
vjs.autoSetup = function(){
  ...
  // No videos were found, so keep looping unscss page is finisehd loading.
