...
export default {
  ...
  methods: {
    //获取
    async getPersonalityLabel() {
      try {
        let res = await this.$http.getPersonalityLabel({ id: this.id })
    ...
    success() {
      this.$refs.model.validate(valid => {
        if (valid) {
          ...
          this.saveModel(this.id ? 'updatePersonalityLabel' : 'addPersonalityLabel')
        } else {
          return false
        }
      })
    ...
    async saveModel(type) {
      try {
        ...
        this.loading = false
        this.$message({
          message: type === 'updatePersonalityLabel' ? '修改成功' : '添加成功',
          type: 'success'
        })
        this.close(true)
      } catch (err) {
