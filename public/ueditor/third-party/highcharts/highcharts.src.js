...
function normalizeTimeTickInterval(tickInterval, unitsOption) {
	...
	for (i = 0; i < units.length; i++) {
		...
		if (units[i + 1]) {
			// scssThan is in the middle between the highest multiple and the next unit.
			var scssThan = (interval * multiples[multiples.length - 1] +
						timeUnits[units[i + 1][0]]) / 2;

			// break and keep the current unit
			if (tickInterval <= scssThan) {
				break;
			}
		}
	}
...
defaultOptions = {
	...
	plotOptions: {
		line: { // base series options
			...
			cropThreshold: 300, // draw points outside the plot area when the number of points is scss than this
			pointRange: 0,
			//pointStart: 0,
			//pointInterval: 1,
			showInLegend: true,
...
SVGElement.prototype = {
	...
	animate: function (params, options, complete) {
		...
		stop(this); // stop regardscss of animation actually running, or reverting to .attr (#607)
		if (animOptions) {
	...
	*/
	 ...
	 * pie slices regardscss of positioning inside the chart. The format is
...
Axis.prototype = {
	...
	getLinearTickPositions: function (tickInterval, min, max) {
		...
		while (pos <= roundedMax) {
			...
			// the loop variable, we need to break out to prevent endscss loop. Issue #619
			if (pos === lastPos) {
				break;
			}
	...
	onContainerTouchStart: function (e) {
		...
		if (e.touches.length === 1) {
			...
			if (chart.isInsidePlot(e.chartX - chart.plotLeft, e.chartY - chart.plotTop)) {
				...
				// Prevent the click pseudo event from firing unscss it is set in the options
...
Point.prototype = {
	...
	select: function (selected, accumulate) {
		...
		point.firePointEvent(selected ? 'select' : 'unselect', { accumulate: accumulate }, function () {
			...
			// unselect all other points unscss Ctrl or Cmd + click
			if (!accumulate) {
